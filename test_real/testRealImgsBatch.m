function testRealImgsBatch(img_db_path, mat_file_base_name, save_mat_flag, fitting_results_flag, colSelect_mode, repeat_times, fitting_results_decided, num_of_col_to_use)
...
for i = 1 : img_cnt
    for idx_repeat = 1 : repeat_times
        ...
        if isequal(colSelect_mode, 'colRandOnOverall')  % random column index set from overall regions            
            region = 'on overall';
        elseif isequal(colSelect_mode, 'colRandOnSmooth') % random column index set from smooth regions
            region = 'on smooth';
        elseif isequal(colSelect_mode, 'colRandOnNonSmooth') % random column index set from nonsmooth regions
            region = 'on nonsmooth';
        end
