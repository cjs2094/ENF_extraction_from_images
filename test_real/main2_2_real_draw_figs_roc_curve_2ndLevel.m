...
eta1_opt_entropy = 0.003; % set by user
...
for kk = 1 : length(enf_strength_levels_reordered)
    ...
    if this_strength ~= 0   
        ...
        eta2_arr = -1:0.0001:1; %threshold; linspace(-2,2,20000);
        TPR_arr = [];
        FPR_arr = [];
        for k = 1 : length(eta2_arr)
            eta2 = eta2_arr(k);
            
            TN_cnt = eta2_dist_100 < eta2;
            FP_cnt = eta2_dist_100 >= eta2;
            TP_cnt = eta2_dist_120 >= eta2;
            FN_cnt = eta2_dist_120 < eta2;       
...
for kk = 1 : length(enf_strength_levels_reordered)
    ...
    if this_strength ~= 0   
        ...
        locations = mse_margin_100 > gamma1_opt_para; 
        ...
        locations = mse_margin_120 > gamma1_opt_para; 
        ...
        %% generate test results
        gamma2_arr = -170:0.01:170; %-8:0.0001:8;
        TPR_arr = [];
        FPR_arr = [];
        for k = 1 : length(gamma2_arr)
            gamma2 = gamma2_arr(k);
            
            TN_cnt = gamma2_dist_100 <  gamma2;
            FP_cnt = gamma2_dist_100 >= gamma2;
            TP_cnt = gamma2_dist_120 >= gamma2;
            FN_cnt = gamma2_dist_120 <  gamma2;
