...
for i = 1 : cnt_enf_strength_levels
    ...
    if this_strength ~= 0
        corr_para_100 = [results_part_100.corr_para];
        corr_para_100 = corr_para_100(:);
        correctDecision_para_100 = [results_part_100.correctDecision_para];
        correctDecision_para_100 = correctDecision_para_100(:);
        
        locations = correctDecision_para_100 == 1;
        corr_para_100_nan = NaN(len, 1);
        corr_para_100_nan(locations) = corr_para_100(locations);
        corr_para_100_nan_arr(:, ii) = corr_para_100_nan;
        
        corr_para_120 = [results_part_120.corr_para];
        corr_para_120 = corr_para_120(:);
        correctDecision_para_120 = [results_part_120.correctDecision_para];
        correctDecision_para_120 = correctDecision_para_120(:);
        
        locations = correctDecision_para_120 == 1;
        corr_para_120_nan = NaN(len, 1);
        corr_para_120_nan(locations) = corr_para_120(locations);
        corr_para_120_nan_arr(:, ii) = corr_para_120_nan;
...
corr_para_arr    = [corr_para_100_nan_arr; corr_para_120_nan_arr];
corr_entropy_arr = [corr_entropy_100_nan_arr; corr_entropy_120_nan_arr];


f = figure;
f.Position = [100 100 540 250];
h1 = boxplot(corr_para_arr, 'Color', plotColors{1}, 'boxstyle', 'filled', 'MedianStyle', 'target',...
...
%b1 = plot(median(rmmissing(corr_para_arr)), 'Color', plotColors{1}, 'Marker', 'o');
