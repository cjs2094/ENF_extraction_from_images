function func_getFreqEstimatesUsingCurveFitting(save_mat_flag, mat_file_name, img_db_path, colSelect_mode, num_of_col_to_use, repeat_times)
...
fitting_results_all = {'img_file_location', 'Fe_set', 'bound_flag', 'fitresult', 'rmse', 'Fe_true', 'Fe_hat_para'};
fitting_results_decided = {'img_file_location', 'Fe_set', 'bound_flag', 'fitresult',...
    'rmse_100Hz', 'rmse_120Hz', 'Fe_true', 'Fe_hat_para', 'col_idx_set', 'region: [m_s, m_t, n_s, n_t]'};
...
for i = 1 : img_cnt
    for idx_repeat = 1 : repeat_times
        ...
        Fe_hat_para_arr = cell2mat(fitting_results_all(ind_for_fitting_results_all - 4 : ind_for_fitting_results_all - 1, 7));
        [~, ind_for_smallest_rmse] = min(rmse_arr);
        rmse_100Hz = min(rmse_arr(1:2));
        rmse_120Hz = min(rmse_arr(3:4));
        Fe_hat_para = Fe_hat_para_arr(ind_for_smallest_rmse);
        ...
        cell_temp = [fitting_results_all(ind_temp, 1:4), rmse_100Hz, rmse_120Hz, ...
            Fe_true, Fe_hat_para, col_idx_set, [m_s, m_t, n_s, n_t]];
        ...
        if save_mat_flag
            save(['.\mat_results\', output_mat_fn], 'fitting_results_all', 'fitting_results_decided');
        else
