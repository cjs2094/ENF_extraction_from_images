function func_genImgsWithEnf(img_path, img_gen_cnt, save_mat_flag, T_row, use_relative_or_absolute_mag_flag, absolute_mag_arr_y, relative_mag, color_channel_mode, mat_file_name, ENF)
...
for i = 1 : img_cnt
    ...
    for j = 1 : length(absolute_mag_arr_y)
        ...
        for k = 1 : img_gen_cnt
            [img_syn, true_param] = func_addSynEnfToImg(img_raw, param);
            ...
            results{cell_ind, 2} = [img_path D(i).name];
...
if save_mat_flag
    mkdir('.\mat_results_syn');
    mat_fn = ['.\mat_results_syn\' mat_file_name];
    if exist(mat_fn)
        delete(mat_fn);
        save(mat_fn, 'results');
    else
